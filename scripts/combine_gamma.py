...
addr='/home/siddharthm/scd/context/800/gamma/'+str(sys.argv[1])#address of the HTK files stored somewhere
cwd='/home/siddharthm/scd/combined/gamma/' #The directory where we will change the address of 
...
num=81*64+1+1 #The length of the feature vector, to be read and stored in the htk format[Right now,20*64 Gammatone+1 Label+1 gender]
file_read='/home/siddharthm/scd/lists/'+str(sys.argv[2]) #The raw filenames, in the form of list
filename='800-gamma-labels-gender-'+str(sys.argv[3]) #The name of the file where stuff is going to be stored
save_extra='800-gamma-extra-'+str(sys.argv[3])+'.txt' #For saving the count of lables
